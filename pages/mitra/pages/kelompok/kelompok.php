<?php
	...
	if(isset($_GET['kelompok'])){
		...
		//$kelompok = $r_kelompok['id_kelompok'];
		
		/*if(!empty($_GET['absen'])) {
			$r_tabsen = mysqli_fetch_array(mysqli_query($dbconnect,"SELECT * FROM tbl_jadwal_monev WHERE id_absen='$_GET[absen]'"));
			$id_absen = $r_tabsen['id_absen'];
		}else{
			echo "";
		}*/
		 
		$r_tjadwal    = mysqli_fetch_array(mysqli_query($dbconnect,"SELECT * FROM tbl_jadwal WHERE id_kelompok='$id_kelompok' AND status_jadwal='monev3'")); 
		
		$statusmonev  = $r_tjadwal['status_jadwal'];
											
		if($statusmonev == "monev3"){
			$mstatus  = "monev3";
		}
		else{
			$mstatus  = "";
	...
	<div class="navbar-collapse collapse" id="navbar-filter">
		<div class="navbar-right">
			<ul class="nav navbar-nav">
				...
				<li class="<?=((empty($r_tjadwal['id_jadwal']) AND empty($mstatus))?'nopointer':'')?>"><a href="?page=nilai-km&absen=<?=$r_tjadwal['id_jadwal']?>&kelompok=<?=$id_kelompok?>&monev=<?=$mstatus;?>"><i class="fa fa-pencil"></i> Penilaian</a></li>
			</ul>
		</div>
	</div>
</div>
								...
								<div class="table-responsive">
									<table class="table table-striped table-bordered table-hover">
										...
										<?php
											...
											$q_jadwal = mysqli_query($dbconnect,"SELECT * FROM tbl_jadwal WHERE id_kelompok='$r_tkelompok[id_kelompok]' AND status_jadwal!='pembekalan'");
										...
										?>
											<tr>
												...
												<td width="30" align="center">
													<a href="?page=absen-mitra&absen=<?=$r_jadwal['id_jadwal']?>" class="label label-default" data-placement="left"  data-popup="tooltip" title="" data-original-title="Cek Absen"><i class="fa fa-search-plus"></i> Cek</a>
