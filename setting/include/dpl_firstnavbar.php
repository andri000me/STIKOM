<?php if(empty($r_dpl['paraf_dpl'])){ ?>
	<div class="alert alert-danger nobor norad" style="margin-bottom:0;">
		<div class="alert-boxed">
		  <strong>Peringatan !</strong> Paraf Anda masih belum dimasukan, silahkan masukan paraf Anda. <a href="?page=profil-dpl&profil=<?=$r_dpl['id_dpl']?>">Klik disini</a> untuk masuk dihalaman profil agar dapat memasukan paraf Anda.
		</div>
	</div>
<?php }else{echo "";}?>	
<!-- Main navbar -->
	...
	<div class="navbar-boxed">
		...
		<div class="navbar-collapse collapse" id="navbar-mobile">	
			<p class="navbar-text"><span class="label bg-success-400"><i class='fa fa-user'></i> Dosen Pembimbing Lapangan</span></p>
			...
			<ul class="nav navbar-nav navbar-right">
				...
				<li class="dropdown dropdown-user <?php if (stripos($_SERVER['REQUEST_URI'], '?page=profil')){echo "active";}?>">
					<a class="dropdown-toggle" data-toggle="dropdown">
						<img src="../../setting/save/dosen/<?=cek_foto($r_dosen['foto_dosen']);?>" alt="">
						<span>Hi, <?=ucwords(cek_jk($r_dosen['jk_dosen'])." ".$namadepan)?></span>
						<i class="fa fa-angle-down"></i>
					</a>

					<ul class="dropdown-menu dropdown-menu-right">
						<li><a href="?page=profil-dpl&profil=<?=$r_dpl['id_dpl']?>"><i class="fa fa-user-circle-o"></i> Profil <?=ucwords(cek_jk($r_dosen['jk_dosen'])." ".$namadepan)?></a></li>
						<li class="divider"></li>
